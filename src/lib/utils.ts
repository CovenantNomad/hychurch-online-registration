...
export function getStartAndEndDateOfThisWeek() {
  ...
  const startDate = new Date(today); // 새 Date 객체를 생성하여 현재 날짜 복사
  startDate.setDate(diffToSunday); // 이번 주 일요일 설정
  startDate.setHours(0, 0, 0, 0); // 일요일 0시 0분 0초로 설정

  const endDate = new Date(startDate); // startDate를 기반으로 새 Date 객체 생성
  endDate.setDate(startDate.getDate() + 6); // 이번 주 토요일(6일 후) 설정
